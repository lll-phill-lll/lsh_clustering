...
public:
    int Load(int argc, char** argv) {
        ...
        app.add_option("--hierarch_level,hierarch_level", hierarch_level, "Max depth of hierarchical clusterization")->capture_default_str();
        app.add_option("--hierarch_clusters,hierarch_clusters", hierarch_clusters, "Stopping criteria for hierarchhical clustering. heirarch_level will be ignored. Stop iterating when number of clusters > hierarchh_clusters")->capture_default_str();
    ...
    // depth of hierarchical mode
    int hierarch_level = 1;
    // stopping criteria for hierarch mode. Stop if clusters > hierarch_clusters
    int hierarch_clusters = 0;
};
